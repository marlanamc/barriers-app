-- Migration: Add two-tier task system (Focus vs Life)
-- Adds task_type and complexity columns to support command center

-- Add task_type column to focus_items
-- Values: 'focus' (capacity-limited) or 'life' (unlimited)
ALTER TABLE focus_items
ADD COLUMN IF NOT EXISTS task_type TEXT DEFAULT 'focus' CHECK (task_type IN ('focus', 'life'));

-- Add complexity column for capacity calculation
-- Values: 'quick' (0.5 pts), 'medium' (1.0 pts), 'deep' (1.5 pts)
ALTER TABLE focus_items
ADD COLUMN IF NOT EXISTS complexity TEXT DEFAULT 'medium' CHECK (complexity IN ('quick', 'medium', 'deep'));

-- Add comment explaining the system
COMMENT ON COLUMN focus_items.task_type IS
'Task type: "focus" (capacity-limited, max 5) or "life" (unlimited, no capacity cost). Focus items are meaningful work, life items are daily essentials like meds, water, meals.';

COMMENT ON COLUMN focus_items.complexity IS
'Task complexity for capacity calculation: "quick" (0.5 pts), "medium" (1.0 pts), "deep" (1.5 pts). Only applies to focus tasks.';

-- Create index for filtering by task type
CREATE INDEX IF NOT EXISTS idx_focus_items_task_type ON focus_items(user_id, task_type, checkin_id);

-- Update existing records to have sensible defaults
-- Set all existing items to 'focus' type with 'medium' complexity
-- Users can adjust these as needed
UPDATE focus_items
SET task_type = 'focus',
    complexity = 'medium'
WHERE task_type IS NULL OR complexity IS NULL;

-- Add the same columns to planned_items for consistency
...
COMMENT ON COLUMN planned_items.task_type IS
'Task type: "focus" (capacity-limited, max 5) or "life" (unlimited, no capacity cost)';

COMMENT ON COLUMN planned_items.complexity IS
'Task complexity for capacity calculation: "quick" (0.5 pts), "medium" (1.0 pts), "deep" (1.5 pts)';

CREATE INDEX IF NOT EXISTS idx_planned_items_task_type ON planned_items(user_id, task_type);

-- Update existing planned items
UPDATE planned_items
SET task_type = 'focus',
    complexity = 'medium'
WHERE task_type IS NULL OR complexity IS NULL;

-- Update the create_checkin_with_focus function to handle task_type and complexity
...
BEGIN
    ...
    IF p_focus_items IS NOT NULL AND jsonb_array_length(p_focus_items) > 0 THEN
        ...
        LOOP
            ...
            -- Handle multiple anchors format
...
'Upserts a check-in with focus items. Supports task_type (focus/life) and complexity (quick/medium/deep) for two-tier task system.';
