-- Migration: Update create_checkin_with_focus to handle completed field
-- This migration updates the RPC function to save and update the completed status of focus items
...
BEGIN
    ...
    IF p_focus_items IS NOT NULL AND jsonb_array_length(p_focus_items) > 0 THEN
        ...
        LOOP
            item_description := COALESCE(trim(focus_record->>'description'), '');
            ...
            WHERE fi.checkin_id = new_checkin_id
              AND fi.description = item_description
            ...
            IF new_focus_item_id IS NOT NULL THEN
                ...
                RAISE NOTICE 'Updated existing focus item % with completed=%', new_focus_item_id, item_completed;
            ...
            ELSE
                ...
                RAISE NOTICE 'Inserted new focus item % with completed=%', new_focus_item_id, item_completed;
            END IF;
...
'Upserts a check-in with focus items. Uses intelligent upsert logic that updates existing items instead of delete+recreate to prevent data loss and improve performance. Supports task_type (focus/life), complexity (quick/medium/deep), completed status, and multiple anchors.';
